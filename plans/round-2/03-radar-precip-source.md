...
> **Status:** 🔬 RESEARCH DELIVERED; **wiring gated on your decision** (2026-06-16) — NOT wired, by design. Reason: the plan's value (an INDEPENDENT radar ground-truth feed) requires a **new third-party external dependency** that would run on every deployed user's widget AND reverses the widget's conservative fail-closed posture toward *confirming* rain — both outward-facing decisions you must own. The no-new-dependency option (Open-Meteo `minutely_15`) is the **same model source**, so it would NOT deliver independent ground-truth (marginal granularity only). **Recommendation:** if you want true radar, the leading CORS-safe/keyless candidate is **RainViewer** (global, dBZ→mm conversion needed) — accept the third-party dependency + define the dBZ threshold; wire it FAIL-CLOSED (missing/stale radar → today's nowcast-only behaviour) and only let it CONFIRM (never fabricate) rain. Decide source + accept the dependency, then this becomes implementable.  ·  **Class:** research  ·  **Priority:** P1  ·  **Effort:** L  ·  **Risk:** low
